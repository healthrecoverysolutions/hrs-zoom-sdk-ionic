...
/*!**************************************************************************************************************************************************************************************!*\
  !*** /Users/anubhavsaxena/Documents/Impetus_Projects/Angular_Monorepo_GitLab_Copy/angular-monorepo/node_modules/@hrsui/angular/node_modules/@hrsui/core/dist/esm/hrs-modal.entry.js ***!
  \**************************************************************************************************************************************************************************************/
/*! exports provided: hrs_modal */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "hrs_modal", function() { return Modal; });
...
const Modal = class {
  ...
  connectedCallback() {
    this.hrsModalDidOpen.emit({
      isOpen: true,
      element: this.el
    });
  }
  /*  For the initial render of a modal, we rely on hrs-modal's componentDidRender(),
   *  which is executed after both hrs-content and hrs-footer have rendered, to calculate
   *  the correct height adjustment for slotted content.
   *  However, for subsequent updates of content within a modal, such as are done for the
   *  wizard, modal will not re-render, only hrs-content will. Therefore, we need to calculate
   *  the height adjustment again as part of hrs-content's componentDidRender().
   */
  componentDidRender() {
    const footer = this.el.querySelector('hrs-footer');
    const content = this.el.querySelector('hrs-content');
    if (footer && content) {
      const footerHeight = footer.clientHeight;
      let subHeaderHeight = 0;
      if (/wizard/i.test(this.el.parentElement.tagName)) {
        const subHeader = this.el.querySelector('.subheader');
        if (subHeader) {
          subHeaderHeight = subHeader.clientHeight;
        }
      }
      content.setAttribute('style', `--siblings-height: ${footerHeight + subHeaderHeight}px`);
      content.parentElement.style.height = '100%';
      content.parentElement.style.overflow = 'hidden';
    }
  }
  disconnectedCallback() {
    this.hrsModalDidClose.emit({
      isOpen: false,
      element: this.el
    });
  }
  handleClick() {
    const functionName = this.canMinimize ? 'toggleMinimize' : 'dismiss';
    if (this.hasNoHeaderButton)
      return;
    this[functionName]();
  }
  toggleMinimize() {
    if (this.isMinimized) {
      this.minClass = 'is-maximizing';
      setTimeout(() => {
        this.minClass = '';
      }, 300);
      this.isMinimized = false;
      ...
      this.minClass = 'is-minimizing';
      setTimeout(() => {
        this.minClass = 'is-minimized';
      }, 300);
      this.isMinimized = true;
    }
    if (this.isMinimized)
      this.hrsMinimizeModal.emit(true);
  }
  dismiss() {
    this.hrsUserSelection.emit(false);
  }
  render() {
    const { hasNoHeaderButton, headerText, canMinimize, minClass, qa } = this;
    return (Object(_index_be85d80d_js__WEBPACK_IMPORTED_MODULE_0__["h"])("hrs-page", { variant: 'modal', "data-qa": qa ? qa : null, class: minClass }, Object(_index_be85d80d_js__WEBPACK_IMPORTED_MODULE_0__["h"])("hrs-header", { "is-modal": true, text: headerText, qa: qa ? `${qa}--header` : 'modal--header' }, hasNoHeaderButton ? null :
      Object(_index_be85d80d_js__WEBPACK_IMPORTED_MODULE_0__["h"])("hrs-button", { slot: "end", variant: "action-icon", icon: canMinimize ? 'minimize' : 'close-pcmt', "icon-color": "inherit", onClick: () => this.handleClick(), qa: qa ? `${qa}--button` : 'modal--button' })), Object(_index_be85d80d_js__WEBPACK_IMPORTED_MODULE_0__["h"])("slot", null)));
  }
  get el() { return Object(_index_be85d80d_js__WEBPACK_IMPORTED_MODULE_0__["g"])(this); }
  static get watchers() { return {
    "canMinimize": ["handleMinimizeUpdate"]
  }; }
};
Modal.style = modalNewDesignCss;



//# sourceMappingURL=hrs-modal.entry.js.map
