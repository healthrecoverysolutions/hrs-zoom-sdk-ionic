...
var AccordionItemComponent = /** @class */ (function () {
    function AccordionItemComponent(renderer) {
...
var HRSComponentsModule = /** @class */ (function () {
    function HRSComponentsModule() {
    }
...
var PatientListPageModule = /** @class */ (function () {
...
var PatientListPage = /** @class */ (function () {
    function PatientListPage(actionSheetCtrl, dataExchangeService, displayGender, http, loadingCtrl, logger, menuCtrl, modalCtrl, navCtrl, patientListService, surveyDetailsService, toastCtrl, translateService, reviewedStatusService, user, platform, zoomService) {
    ...
    PatientListPage_1 = PatientListPage;
    /**
    ...
    PatientListPage.prototype.ngOnInit = function () {
        ...
        this.searchControl.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["debounceTime"])(700)).subscribe(function (search) {
            _this.searching = false;
            _this.filterPatients();
        });
        this.expandedPatients = this.patientListService.expandedPatients();
    };
    PatientListPage.prototype.ionViewDidEnter = function () {
    ...
    PatientListPage.prototype.ionViewWillLeave = function () {
     ...
     */
    PatientListPage.prototype.getPatientList = function (patientListLoading, refresher) {
        var _this = this;
        var patients = [];
        ...
        return this.patientListService.loadPatients().subscribe(function (patientList) {
            ...
            for (var i = 0; i < patientList.length; i++) {
                var patient = patientList[i][0];
                _this.patientFullProfile = new _clinician_models__WEBPACK_IMPORTED_MODULE_11__["Patient"](patient.profile);
                _this.patientFullProfile.expanded = false;
                _this.patientFullProfile.metrics = patient.metrics;
                _this.patientFullProfile.risk = patient.risk;
                _this.patientFullProfile.reviewedState = _this.patientFullProfile.reviewed && _this.patientFullProfile.reviewed[0] ?
                    _this.patientFullProfile.reviewed[0].reviewedStatus : _clinician_providers__WEBPACK_IMPORTED_MODULE_8__["ReviewedStatusService"].NON_REVIEWED;
                patients.push(_this.patientFullProfile);
            }
            // allPatients is never changed and used to reset the patient list after a search
            _this.allPatients = patients.slice();
            // visible patients are the patients that are shown in the list changes with when a search term is applied
            _this.visiblePatients = _this.allPatients.slice();
            if (patientListLoading) {
                _this.hideLoader();
                _this.patientListLoading = false;
            }
            if (refresher) {
     ...
     */
    PatientListPage.prototype.updatePatient = function () {
        var _a = this.dataExchangeService.getData(_clinician_providers__WEBPACK_IMPORTED_MODULE_8__["DataExchangeService"].UpdatePatient, true), hrsid = _a.hrsid, conditions = _a.conditions, metrics = _a.metrics;
        var visPatient = this.visiblePatients.filter(function (p) { return p.hrsid === hrsid; })[0];
    ...
    PatientListPage.prototype.updateReviewedStatus = function () {
        var _a = this.dataExchangeService.getData(_clinician_providers__WEBPACK_IMPORTED_MODULE_8__["DataExchangeService"].UpdatedReviewedStatus, true), hrsid = _a.hrsid, reviewedStatus = _a.reviewedStatus;
        var visPatient = this.visiblePatients.filter(function (patient) { return patient.hrsid === hrsid; })[0];
    ...
    PatientListPage.prototype.updateActivationStatus = function () {
        var _a = this.dataExchangeService.getData(_clinician_providers__WEBPACK_IMPORTED_MODULE_8__["DataExchangeService"].UpdatedActivationStatus, true), hrsid = _a.hrsid, status = _a.status, discharged = _a.discharged;
        var visPatient = this.visiblePatients.filter(function (patient) { return patient.hrsid === hrsid; })[0];
     ...
     */
    PatientListPage.prototype.showLoader = function () {
    ...
    PatientListPage.prototype.hideLoader = function () {
        var _this = this;
        this.loadingCtrl.getTop().then(function (element) {
            if (element) {
     ...
     */
    PatientListPage.prototype.expandPatient = function (patient) {
        var _this = this;
        var rerender = function () {
            ...
            _this.patientListScrollElement.checkRange(0, _this.visiblePatients.length);
        };
        var unexpand = function () {
            _this.expandedPatients.unset();
            rerender();
        };
     ...
     */
    PatientListPage.prototype.itemHeight = function (patient, index) {
        return patient && patient.expanded ? 400 : 150;
    };
    /**
     *  Parse risk in order to apply risk coloring classes to metrics
     */
    PatientListPage.prototype.parsePatientRisk = function (_, patient) {
        if (patient.risk && patient.risk[0]) {
            var patientRisk = patient.risk[0];
            if (patientRisk.details && patientRisk.details.length > 0) {
                var riskDetails = patientRisk.details;
                for (var i = 0; i < patientRisk.details.length; i++) {
                    var riskLevel = riskDetails[i].type;
                    if (riskDetails[i].metrics) {
                        patient[riskDetails[i].metrics[0] + 'Risk'] = riskLevel;
                    }
                }
            }
            var risk = PatientListPage_1.getRiskText(patientRisk);
     ...
     */
    PatientListPage.getRiskText = function (risk) {
        var riskLevel = risk.value;
     ...
     */
    PatientListPage.prototype.calculatePatientMedications = function (patient) {
        var medications;
        var totalMedications = 0;
        var medicationsTakenToday = [];
     ...
     */
    PatientListPage.prototype.viewPatientProfile = function (event, patient) {
        var _this = this;
        ...
        this.navCtrl.navigateForward('/patient-profile', navigationExtras).then(function () {
     ...
     */
    PatientListPage.prototype.submitNewReviewedStatus = function (event, patient) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var requestedStatus;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                ...
                requestedStatus = this.reviewedStatusService.requestedReviewedStatus(patient.reviewedState);
                return [2 /*return*/, this.reviewedStatusService.submitRequestedStatus(patient.hrsid, requestedStatus).subscribe({
                        next: function (resp) {
                            patient.reviewLoading = false;
                            patient.reviewedState = requestedStatus;
                            patient.reviewed = [{ name: _this.user.getClinicianName(), time: Date.now().toString(), reviewedStatus: requestedStatus }];
                            var message = _this.translateService.instant('REVIEWED_STATUS_SUBMITTED_SUCCESS', {
                                patientName: patient.name.first + ' ' + patient.name.last,
                                newStatus: _this.reviewedStatusService.translateReviewedStatus(patient.reviewedState),
                            });
                            _this.throwReviewToast(message, _this.reviewedStatusService.popupReviewedStatusClass(patient.reviewedState));
                        },
                        error: function (err) {
                            patient.reviewLoading = false;
                            var message = _this.translateService.instant('REVIEWED_STATUS_SUBMITTED_ERROR', {
                                patientName: patient.name.first + ' ' + patient.name.last,
                                reviewedStatus: _this.reviewedStatusService.translateReviewedStatus(requestedStatus)
                            });
                            _this.throwReviewToast(message, _this.reviewedStatusService.popupReviewedStatusClass());
    ...
    PatientListPage.prototype.throwReviewToast = function (message, cssClass) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var toast;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
     ...
     */
    PatientListPage.prototype.openCommunicationActionSheet = function (event, patient) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var actionSheet;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        this.translateService.get('CANCEL_BUTTON').subscribe(function (value) {
                            _this.cancelMessage = value;
                        });
                        this.translateService.get('COMMUNICATION_BUTTON').subscribe(function (value) {
                            _this.communicationButton = value;
                        });
                        this.translateService.get('VIDEO_BUTTON').subscribe(function (value) {
                            _this.videoButton = value;
                        });
                        this.translateService.get('VOICE_BUTTON').subscribe(function (value) {
                            _this.voiceButton = value;
                        });
                        this.translateService.get('CHAT_BUTTON').subscribe(function (value) {
                            _this.chatButton = value;
                        });
                        return [4 /*yield*/, this.actionSheetCtrl.create({
                                ...
                                buttons: [
                                    ...
                                    },
                                    {
                                        ...
                                        handler: function () {
                                            _this.openChatMessages(patient);
                                        }
                                    },
                                    {
     ...
     */
    PatientListPage.prototype.openChatMessages = function (patient) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var modal;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
     ...
     */
    PatientListPage.prototype.openVideoCall = function (patient) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var patientName, modal;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        patientName = patient.name.first + ' ' + patient.name.last;
     ...
     */
    PatientListPage.prototype.openVoiceCall = function (patient) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var riskClass, patientName, modal;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
     ...
     */
    PatientListPage.prototype.getSurveyQuestionAndAnswerMetric = function (patient) {
        var _this = this;
        this.surveyDetailsService.getSurveyDetails(patient.hrsid, moment__WEBPACK_IMPORTED_MODULE_10___default()().format('YYYY-MM-DD')).subscribe({
            next: function (dailySurveyDetails) {
     ...
     */
    PatientListPage.prototype.toggleSearch = function () {
        this.search = !this.search;
        if (!this.search) {
            this.searchTerm = '';
            this.previousSearchTerm = '';
            // reset patient list to show all patients when closing search
            var patients = this.allPatients.slice();
     ...
     */
    PatientListPage.prototype.onSearchInput = function (ev) {
        this.searching = true;
    };
    /**
     * Filter patient list by first name last name full name or PID
     */
    PatientListPage.prototype.filterPatients = function () {
        var _this = this;
        // search term was changed or removed
        if (this.searchTerm !== this.previousSearchTerm || this.searchTerm === '') {
            var allPatients = this.allPatients.slice();
            this.visiblePatients = allPatients;
        }
        if (this.visiblePatients && this.searchTerm.length > 1) {
            var allPatients = this.allPatients.slice();
            this.visiblePatients = allPatients.filter(function (patient) {
                var isMatch = false;
                if (patient.name.first) {
                    // match just first name
                    isMatch = isMatch || patient.name.first.toLowerCase().indexOf(_this.searchTerm.toLowerCase().replace(/\s/g, '')) > -1;
                }
                if (patient.name.last) {
                    // match just last name
                    isMatch = isMatch || patient.name.last.toLowerCase().indexOf(_this.searchTerm.toLowerCase().replace(/\s/g, '')) > -1;
                }
                if (patient.name.first && patient.name.last) {
                    // match first and last ignoring whitespace
                    isMatch = isMatch || ((patient.name.first + patient.name.last).toLowerCase()).indexOf(_this.searchTerm.toLowerCase().replace(/\s/g, '')) > -1;
                }
                if (patient.pid) {
                    // match patient id
                    isMatch = isMatch || patient.pid.toLowerCase().indexOf(_this.searchTerm.toLowerCase().replace(/\s/g, '')) > -1;
                }
                return isMatch;
            });
        }
    ...
    PatientListPage.prototype.canContactPatient = function () {
        return this.user.canContactPatient();
    };
    PatientListPage.prototype.canEditPatientInfo = function () {
        return this.user.canEditPatientInfo();
    };
    var PatientListPage_1;
    PatientListPage.ctorParameters = function () { return [
